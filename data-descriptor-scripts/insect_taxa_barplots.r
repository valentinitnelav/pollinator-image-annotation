...
dt_taxat_hym <- dt[order == what_order, .(n_box = .N), keyby = taxa_cols]
...
n_apis <- dt_taxat_hym[species == "mellifera", n_box]
...
gg_hym <- ggplot(appended_data, aes(x = Variable, y = Count, fill = Species)) +
  ...
  scale_y_continuous(labels = comma_format(),
                     sec.axis = sec_axis(trans = ~ . / order_count * 100, 
                                         name = "%",
...
dt_taxat_dip <- dt[order == what_order, .(n_box = .N), keyby = taxa_cols]
...
# 2: family/family-cluster  3066   51.41707
# 3:                 genus  1597   26.78182
# 4:        genera-cluster  1206   20.22472
# 5:               species   914   15.32786

# Prepare a custom stacked barplot

# Initialize a data table for Myathropa florea and other species at the species level
dominant_sp <- "Myathropa florea"
n_mflo <- dt_taxat_dip[species == "florea", n_box]
n_sp <- non_na_counts$species
species_count <- data.table(Variable = rep("species", 2),
